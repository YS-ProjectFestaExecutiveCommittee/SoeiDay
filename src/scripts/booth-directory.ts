...
function openBoothDetail(id, updateHash = true) {
	...
	const imageMarkup = booth.imageData
		? booth.boothUrl
			? `<a href="${escapeHtml(booth.boothUrl)}" target="_blank" rel="noopener noreferrer" class="block group booth-detail-media" aria-label="${escapeHtml(booth.title)}の詳細ページへ"><img src="${escapeHtml(booth.imageData)}" alt="${escapeHtml(booth.title)}" class="bg-slate-100 rounded-sm transition-transform duration-500 group-hover:scale-[1.01]"><span class="block text-center text-xs tracking-widest text-brand-orange py-2 font-sans">画像をタップして専用ページへ <i class="fa-solid fa-arrow-up-right-from-square ml-1"></i></span></a>`
			: `<div class="booth-detail-media"><img src="${escapeHtml(booth.imageData)}" alt="${escapeHtml(booth.title)}" class="bg-slate-100 rounded-sm"></div>`
		: `<div class="booth-detail-media flex items-center justify-center bg-slate-100 rounded-sm text-slate-400"><i class="fa-solid fa-store text-5xl"></i></div>`;

	const tagsHtml = Array.isArray(booth.tags)
		? booth.tags
				.map(
					(tag) =>
						`<span class="px-2.5 py-1 bg-gray-100 text-xs text-slate-600 rounded-sm">#${escapeHtml(tag)}</span>`,
				)
				.join("")
		: "";
	const videoHtml = booth.youtubeId
		? `<div class="mt-8"><div class="aspect-video w-full overflow-hidden bg-black shadow-lg"><iframe src="${youtubeEmbedUrl(booth.youtubeId)}" title="${escapeHtml(booth.title)} YouTube動画" class="w-full h-full border-0" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div></div>`
		: "";

	detailView.innerHTML = `
    <div class="bg-white shadow-xl border border-gray-100 rounded-sm overflow-hidden">
      <div class="booth-detail-grid gap-8 lg:gap-10 p-5 sm:p-8 lg:p-10">
        <div class="booth-detail-image-wrap rounded-sm border border-gray-100 p-3 self-start">${imageMarkup}</div>
        <div class="min-w-0">
          <div class="flex flex-wrap items-center gap-2 mb-4">
            <span class="px-3 py-1 ${meta.bg} ${meta.text} text-xs tracking-widest rounded-sm">${meta.label}</span>
            <span class="text-sm text-slate-500 tracking-wider"><i class="fa-solid fa-location-dot mr-1"></i>${escapeHtml(booth.roomName)}</span>
            <span class="text-sm text-slate-500 tracking-wider"><i class="fa-solid fa-layer-group mr-1"></i>${escapeHtml(FLOOR_TITLES[booth.floor] || booth.floor)}</span>
          </div>
          <h2 class="text-3xl md:text-4xl font-bold tracking-widest leading-tight text-slate-900 border-b border-gray-100 pb-5">${escapeHtml(booth.title)}</h2>
          ${organizer ? `<div class="mt-5 flex items-center gap-2 text-slate-500"><i class="fa-solid fa-users text-sm"></i><span class="text-sm tracking-widest">${escapeHtml(organizer)}</span></div>` : ""}
          <div class="mt-7 text-slate-700 leading-loose tracking-wide whitespace-pre-wrap text-sm sm:text-base">${escapeHtml(booth.description)}</div>
          ...
          <div class="mt-10 pt-6 border-t border-gray-100 flex flex-wrap gap-3">
            <button type="button" id="back-to-booth-list" class="inline-flex items-center gap-2 border border-slate-900 px-6 py-3 tracking-widest text-sm hover:bg-slate-900 hover:text-white transition"><i class="fa-solid fa-arrow-left"></i>一覧へ戻る</button>
            <a href="/map/#${encodeURIComponent(id)}" class="inline-flex items-center gap-2 border border-slate-300 px-6 py-3 tracking-widest text-sm hover:border-brand-orange transition"><i class="fa-solid fa-map-location-dot"></i>会場マップで見る</a>
          </div>
        </div>
      </div>
    </div>
  `;
	document
		.getElementById("back-to-booth-list")
		.addEventListener("click", () => showBoothList());
...
function renderBoothList(searchTerm = "") {
	...
	const normalizedTerm = String(searchTerm || "")
		.trim()
		.toLocaleLowerCase("ja");
	const entries = Object.entries(booths).filter(
		([, booth]) =>
			!normalizedTerm || getSearchableText(booth).includes(normalizedTerm),
	);
	const summary = document.getElementById("booth-search-summary");
	const clearButton = document.getElementById("booth-search-clear");
	if (summary)
		summary.textContent = normalizedTerm
			? `${entries.length}件が見つかりました。`
			: `${Object.keys(booths).length}件を掲載しています。`;
	if (clearButton) clearButton.classList.toggle("hidden", !normalizedTerm);

	container.innerHTML = floorOrder
		.map((floorKey) => {
			const floorBooths = entries
				.filter(([, booth]) => booth.floor === floorKey)
				.sort(([, a], [, b]) =>
					String(a.title).localeCompare(String(b.title), "ja"),
				);
			if (!floorBooths.length) return "";
			const cards = floorBooths
				.map(([id, booth]) => {
					const meta = categoryMetaOf(booth.category);
					const organizer = getOrganizerName(booth);
					const tags = Array.isArray(booth.tags)
						? booth.tags
								.slice(0, 4)
								.map(
									(tag) =>
										`<span class="px-2 py-0.5 bg-gray-100 text-[10px] text-slate-600 rounded">#${escapeHtml(tag)}</span>`,
								)
								.join("")
						: "";
					return `
        <button type="button" data-booth-id="${escapeHtml(id)}" class="group text-left bg-white border border-gray-100 shadow-lg rounded-sm overflow-hidden hover:-translate-y-0.5 transition-transform duration-300">
          <div class="grid grid-cols-1 sm:grid-cols-[170px_1fr]">
            <div class="h-44 sm:h-full min-h-[150px] bg-slate-100 overflow-hidden">
            ...
            <div class="p-5">
              ...
              <h3 class="text-xl font-bold tracking-widest text-slate-900 leading-relaxed">${escapeHtml(booth.title)}</h3>
              ${organizer ? `<p class="mt-1 text-xs text-slate-500"><i class="fa-solid fa-users mr-1"></i>${escapeHtml(organizer)}</p>` : ""}
              <p class="mt-3 text-sm leading-relaxed text-slate-600 font-light">${escapeHtml(summaryText(booth.description))}</p>
              ${tags ? `<div class="flex flex-wrap gap-1.5 mt-4">${tags}</div>` : ""}
              <div class="mt-5 text-brand-orange text-xs tracking-widest font-sans">詳細を見る <i class="fa-solid fa-arrow-right ml-1 group-hover:translate-x-1 transition-transform"></i></div>
            </div>
          </div>
        </button>`;
				})
      ...
      <section class="floor-section">
        <div class="flex items-end gap-4 mb-6 border-b-2 ${floorKey === "h1" || floorKey === "h3" || floorKey === "s2" ? "border-brand-orange" : "border-brand-lime"} pb-3">
          <h3 class="text-2xl font-bold tracking-widest text-slate-900">${escapeHtml(FLOOR_TITLES[floorKey])}</h3>
          <span class="text-xs text-slate-500 font-sans tracking-widest">${floorBooths.length} 件</span>
        </div>
	...
	container
		...
		.forEach((button) =>
			button.addEventListener("click", () =>
				openBoothDetail(button.dataset.boothId),
			),
		);
}
...
function renderMapPins() {
	const surface =
		renderedMap?.querySelector("#map-click-surface") || renderedMap;
...
function renderMapDetail(booth, id) {
	...
	const tags = Array.isArray(booth.tags)
		? booth.tags
				.map(
					(tag) =>
						`<span class="text-[11px] px-2 py-0.5 bg-gray-100 text-slate-600 rounded">#${escapeHtml(tag)}</span>`,
				)
				.join("")
		: "";
    ...
    <div class="space-y-4 animate-fade-in">
      ${booth.imageData ? `<button type="button" id="map-detail-image" class="block w-full overflow-hidden bg-slate-100 rounded-sm"><img src="${escapeHtml(booth.imageData)}" alt="${escapeHtml(booth.title)}" class="w-full max-h-56 object-contain"></button>` : ""}
      ...
      <h3 class="text-xl font-bold text-slate-900 border-b border-gray-100 pb-2">${escapeHtml(booth.title)}</h3>
      <p class="text-sm text-slate-600 font-light leading-relaxed whitespace-pre-wrap">${escapeHtml(booth.description)}</p>
      ${tags ? `<div class="flex flex-wrap gap-1 pt-2">${tags}</div>` : ""}
      <a href="/booth/#${encodeURIComponent(id)}" class="inline-flex items-center gap-2 text-brand-orange text-sm tracking-widest">出店詳細を見る <i class="fa-solid fa-arrow-right"></i></a>
    </div>`;
...
async function switchFloor(floorKey) {
	...
	if (el.floorTitle)
		el.floorTitle.textContent = `${FLOOR_TITLES[floorKey]} フロアマップ`;
	try {
		...
		el.mapTarget.innerHTML =
			'<div class="map-error flex items-center justify-center text-center text-red-500"><div><i class="fa-solid fa-triangle-exclamation text-3xl mb-3"></i><p class="text-sm leading-relaxed">フロア図を読み込めませんでした。<br>Blueprintファイルの読み込みを確認してください。</p></div></div>';
	}
}
...
async function loadBooths() {
	const q = query(
		collection(db, BOOTHS_COLLECTION),
...
loadBooths().catch((error) => {
	...
	if (el.loading)
		el.loading.innerHTML =
			'<div class="text-red-500">出店情報を読み込めませんでした。Firebaseの設定とRulesを確認してください。</div>';
});
