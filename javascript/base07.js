...
function makeNewsRow(item) {
  ...
  anchor.className =
    "group block border-b border-gray-200 dark:border-white/10 py-5 hover:opacity-75 transition-opacity";
  ...
  date.className =
    "text-xs text-slate-500 dark:text-slate-400 font-sans tracking-wider";
  ...
  badge.textContent = item.type === "news_release"
    ? "ニュースリリース"
    : "トピック";
  ...
  title.className =
    "text-base md:text-lg font-bold tracking-wide text-slate-900 dark:text-white";
  title.textContent = item.title || "無題の投稿";

  const excerpt = document.createElement("p");
  excerpt.className =
    "text-xs md:text-sm text-slate-600 dark:text-slate-400 mt-1.5 leading-relaxed";
  excerpt.textContent = excerptFromHtml(item.content);

  const numberLabel = document.createElement("p");
  numberLabel.className =
    "text-[10px] font-sans tracking-widest text-slate-400 dark:text-slate-500 mt-2";
