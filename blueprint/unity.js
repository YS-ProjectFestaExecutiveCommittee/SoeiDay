...
const BLUEPRINT_URLS = {
  h1: 'https://www.soei-fes.com/blueprint/h1.js',
  h2: 'https://www.soei-fes.com/blueprint/h2.js',
  h3: 'https://www.soei-fes.com/blueprint/h3.js',
  s1: 'https://www.soei-fes.com/blueprint/s1.js',
  s2: 'https://www.soei-fes.com/blueprint/s2.js',
  s3: 'https://www.soei-fes.com/blueprint/s3.js'
};

export { FLOOR_TITLES, BLUEPRINT_URLS };
...
async function fetchBlueprintData(floorKey) {
  const response = await fetch(BLUEPRINT_URLS[floorKey], { cache: 'no-store' });
...
function renderBlueprintPaths(target, mapDataObj, floorKey) {
  ...
  if (!Array.isArray(items)) throw new Error('フロア図の配列データを確認できませんでした。');

  const pathsHtml = items.map((item) => {
    const attrs = Object.entries(item?.attributes || {})
      .map(([key, value]) => `${escapeHtml(key)}="${escapeHtml(value)}"`)
      .join(' ');
    const tagName = item?.type || 'path';
    const safeTag = ['path', 'rect', 'line', 'polyline', 'polygon', 'circle', 'ellipse', 'text'].includes(tagName) ? tagName : 'path';
...
export async function renderBlueprint(target, floorKey) {
  ...
  shell.className = 'w-full h-full relative flex items-center justify-center min-h-[420px] bg-white dark:bg-slate-900 border border-dashed border-gray-300 dark:border-slate-700 p-4 rounded-sm';
  target.innerHTML = '';
...
export function positionElementFromPercent(element, layer, svg, posX, posY) {
  ...
  if (!Number.isFinite(x) || !Number.isFinite(y) || !svg || !layer || !svg.viewBox?.baseVal || typeof svg.getScreenCTM !== 'function') {
...
export function percentFromPointerEvent(event, svg) {
  if (!svg || !svg.viewBox?.baseVal || typeof svg.getScreenCTM !== 'function') return null;
  const ctm = svg.getScreenCTM();
  if (!ctm) return null;
  const point = svg.createSVGPoint();
...
export function addResizeRepositionListener(callback) {
  window.addEventListener('resize', () => requestAnimationFrame(callback), { passive: true });
}
