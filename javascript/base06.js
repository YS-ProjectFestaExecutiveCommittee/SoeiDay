...
if (Math.abs(lastSignInTime - creationTime) < 5000 && user.providerData.some(p=> p.providerId === 'google.com')) {
    ...
    document.getElementById('news-list-table').innerHTML = `<tr>
        <td colspan="4" class="p-8 text-center text-red-500">データ取得エラー: ${error.message}</td>
    </tr>`;
    });
    };
    ...
    if (data.length === 0) {
    tbody.innerHTML = '<tr>
        <td colspan="4" class="p-8 text-center text-gray-500">記事がありません。</td>
    </tr>';
    return;
    }
